const ItemSingleMateriaService = require('../services/ItemSingleMateriaService');
...
const getTopics = async (req, res) => {
  ...
  try {
    const result = await ItemSingleMateriaService.getAllTopics(
      pageNumber,
      limitNumber,
      search,
      validSort
...
const addTopic = async (req, res) => {
  ...
  try {
    const newTopic = await ItemSingleMateriaService.addTopic({ nomeTopico, numeroArquivos });
...
const deleteTopic = async (req, res) => {
  ...
  try {
    const wasDeleted = await ItemSingleMateriaService.deleteTopic(id);
    if (wasDeleted) {
