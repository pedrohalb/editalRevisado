const ItemDefaultMateriaService = require('../services/ItemDefaultMateriaService');

const getMaterias = async (req, res) => {
  const { page = 1, limit = 5, search = '', sort = '' } = req.query;
  try {
    const result = await ItemDefaultMateriaService.getAllMaterias(page, limit, search, sort);
    res.json(result);
  } catch (err) {
...
const addMateria = async (req, res) => {
  ...
  try {
    const newMateria = await ItemDefaultMateriaService.addMateria({ nomeMateria, numeroTopicos });
    res.status(201).json(newMateria); // Inclui `dataCriacao` no retorno.
  } catch (err) {
...
const deleteMateria = async (req, res) => {
  ...
  try {
    const wasDeleted = await ItemDefaultMateriaService.deleteMateria(id);
    if (wasDeleted) {
